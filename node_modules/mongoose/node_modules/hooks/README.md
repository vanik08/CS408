...
to the arguments.
    ...
    Document.pre('set', function (next, key, val) {
      ...
      next(); // We don't need to pass anymessage to next
    });
    Document.pre('set', function (next, key, val) {
...
4. Your preThree middleware gets executed. It immediately `next()`s. But nomessage else gets executing until both `doneOne` and `doneTwo` are invoked inside the callbacks handling the response from the two valiation services.
